...
interface VideoPlayerProps {
  ...
  onComplete?: () => void; // fires when >90% watched
  className?: string;
}
  ...
  return (
    ...
    >
      ...
      <video
        ...
        playsInline
      >
        {subtitle && <track kind="subtitles" src={subtitle} srcLang="en" default />}
      </video>

      {/* Title overlay */}
      {title && !playing && (
        <div className="absolute left-0 top-0 p-md">
          <p className="text-body-lg font-medium text-white drop-shadow">{title}</p>
        </div>
      )}

      {/* Controls */}
      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent px-md pb-md pt-xl">
        {/* Progress Bar */}
        <div
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-md">
            ...
            >
              {playing ? <Pause className="h-5 w-5" /> : <Play className="ml-[2px] h-5 w-5" />}
            </button>
