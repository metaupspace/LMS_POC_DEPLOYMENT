...
export default function CreateStaffPage() {
  ...
  const onSubmit = async (formData: CreateUserInput) => {
    setServerError('');
    try {
      await createUser({ ...formData, role: 'staff' }).unwrap();
      dispatch(addToast({ type: 'success', message: 'Staff member created successfully.', duration: 3000 }));
      router.push('/admin/staff');
    } catch (err) {
      const message = (err as { data?: { message?: string } })?.data?.message ?? 'Failed to create staff member.';
  ...
  return (
    <div className="flex flex-col gap-lg">
      ...
      <div className="flex items-center gap-md">
        ...
        <h1 className="text-h1 text-text-primary">Add Staff Member</h1>
      </div>
